...
# note: output is
# data/local/lm/4gram-mincount/lm_unpruned.gz
...
cat $cleantext | awk '{for(n=2;n<=NF;n++) print $n; }' | sort -u > $dir/wordlist || exit 1;

cat $cleantext | awk '{for(n=2;n<=NF;n++){ printf $n; if(n<NF) printf " "; else print ""; }}' > $dir/train

ngram-count -text $dir/train -order 5 -limit-vocab -vocab $dir/wordlist -unk \
  -map-unk "<UNK>" -kndiscount -interpolate -lm $dir/srilm.o5g.kn.gz
echo "local/train_lms.sh succeeded"
exit 0
