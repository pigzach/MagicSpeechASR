...
dir=exp/rnnlm_lstm_tdnn_pretrain
lstm_rpd=100
lstm_nrpd=100
...
text_dir=data/rnnlm_pretrain/text
...
if [ $stage -le 0 ]; then
  ...
  awk -v text_dir=$text_dir '{if(NR%500 == 0) { print >text_dir"/dev.txt"; } else {print;}}' >$text_dir/magic.txt
fi
